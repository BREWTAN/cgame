...
import onight.tfw.rest.web.bind.QueryMapperResolver;
...
public class SqlMaker {
	...
	public static String selectSql(String selectInfo ,DbCondi dc) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
		Class<?> clazz = dc.getEntityClass();
		QueryMapperBean queryCondi = dc.getQmb();
		PageInfo para = dc.getPageinfo();
		String  sort = para.getSort();
		...
		if(selectInfo==null||selectInfo.trim().equals("")){
			FieldsMapperBean fmb=dc.getFmb();
			selectInfo=getDbFieldNames(clazz, fmb);
		}
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT ").append(selectInfo).append(" FROM ");
		sql.append(getTableName(clazz, dc));
