...
CREATE TABLE TLT_USER_BANK (
    ...
    ENABLE int(1) NOT NULL DEFAULT '1' COMMENT '是否有效 1有效 0无效',
    PRIMARY KEY (ID)
...
CREATE TABLE TLT_LTYPE (
    ...
    SORT int(4) NOT NULL DEFAULT '0' COMMENT '排序，一般显示用',
    ENABLE int(1) NOT NULL DEFAULT '0' COMMENT '0停售 1销售中',
...
CREATE TABLE TLT_PLAY_TYPE (
    ...
    LOCK_STATUS int(1) DEFAULT '0' COMMENT '封锁状态，0未封锁 1封锁',
    MODE_YJFL varchar(16) DEFAULT '1,2,3,4' COMMENT '元角模式，1支持元,2支持角 3支持分 4支持厘',
    ISANDVALUE int(1) DEFAULT '0' COMMENT '是否为和值玩法，0否 1是',
...
CREATE TABLE TLT_ISSUE_GEN_PARAMS (
  ...
  SORT int(4) NOT NULL DEFAULT '0' COMMENT '排序，一般显示用',
  ENABLE int(1) NOT NULL DEFAULT '0' COMMENT '0无效 1有效',
  PRIMARY KEY (GEN_ID)
)  ENGINE=INNODB DEFAULT CHARSET=UTF8 COMMENT='期号时间设置表，是批量生成期号的基础';
...
CREATE INDEX INX_T_CORE_TICK_LTYPE ON TLT_CORE_TICKET
(
   LTYPE
);

CREATE INDEX INX_T_CORE_TICK_ISSUE_NO ON TLT_CORE_TICKET
(
   ISSUE_NO
);
...
CREATE INDEX INX_T_COREBET_TO_LTYPE_PTYPE ON TLT_CORE_BET
(
   LTYPE,PLAY_TYPE
);

CREATE INDEX INX_T_COREBET_TO_SERIALNUM ON TLT_CORE_BET
(
   SERIAL_NUM
);
CREATE INDEX INX_T_COREBET_TO_ISSUE_NO ON TLT_CORE_BET
(
   ISSUE_NO
);
