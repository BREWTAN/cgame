...
object BetsCalc extends Calc {
  ...
  val playTypessMap: HashMap[String, CalcMethod] = HashMap[String, CalcMethod](
    ...
    "后三码复式" -> calc_后三码复式,
    "后三码单式" -> calc_后三码复式,
    "后三码直选和值" -> calc_后三码直选和值,
    "后三码组三" -> calc_后三码组三,
    "后三码组六" -> calc_后三码组六,
    "后三码混合组选" -> calc_后三码组六,
    "后三码组选和值" -> calc_后三码组选和值,

    "前三码复式" -> calc_前三码复式,
    "前三码单式" -> calc_前三码复式,
    "前三码直选和值" -> calc_前三码直选和值,
    "前三码组三" -> calc_前三码组三,
    "前三码组六" -> calc_前三码组六,
    "前三码混合组选" -> calc_前三码组六,
    "前三码组选和值" -> calc_前三码组选和值,

    "中三码复式" -> calc_中三码复式,
    "中三码单式" -> calc_中三码复式,
    "中三码直选和值" -> calc_中三码直选和值,
    "中三码组三" -> calc_中三码组三,
    "中三码组六" -> calc_中三码组六,
    "中三码混合组选" -> calc_中三码组六,
    "中三码组选和值" -> calc_中三码组选和值,

    "二码后二直选(复式)" -> calc_二码后二直选复式,
    "二码后二直选(单式)" -> calc_二码后二直选复式,
    "二码前二直选(复式)" -> calc_二码前二直选复式,
    "二码前二直选(单式)" -> calc_二码前二直选复式,
    "二码后二直选和值" -> calc_二码后二直选和值,
    "二码前二直选和值" -> calc_二码前二直选和值,

    "二码后二组选(复式)" -> calc_后二码二组选复式,
    "二码后二组选(单式)" -> calc_后二码二组选复式,
    "二码前二组选(复式)" -> calc_前二码二组选复式,
    "二码前二组选(单式)" -> calc_前二码二组选复式,
    "二码后二组选和值" -> calc_二码后二组选和值,
    "二码前二组选和值" -> calc_二码前二组选和值,
    "定位胆定位胆" -> calc_定位胆,

    "不定胆后三一码不定胆" -> calc_不定胆后一码不定胆,
    "不定胆后三二码不定胆" -> calc_不定胆后二码不定胆,
    "不定胆前三一码不定胆" -> calc_不定胆前一码不定胆,
    "不定胆前三二码不定胆" -> calc_不定胆前二码不定胆,

    "大小单双前大小单双" -> calc_大小单双前大小单双,
    "大小单双后大小单双" -> calc_大小单双后大小单双,
    "一帆风顺" -> calc_一帆风顺,
    "好事成双" -> calc_好事成双,
    "三星报喜" -> calc_三星报喜,
    "四季发财" -> calc_四季发财
