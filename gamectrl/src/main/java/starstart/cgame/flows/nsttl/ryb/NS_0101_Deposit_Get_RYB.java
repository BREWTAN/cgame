...
/**
 * 
 * @ClassName: NS_0101_Deposit_Get_RYB
 * @Description: 获取产品方（如意宝）申购文件
...
@Slf4j
@Data
public class NS_0101_Deposit_Get_RYB extends NSettle_BaseActor<CalcMutex> {
	...
	public void processMessage(CalcMutex pack) {
		...
		log.info("STTL:处理获取RYB申购文件" + pack);
		try {
			...
			//根据产品方（如意宝）提供的商户号获取申购文件
			if(null!=channellist&&channellist.size()>0){
				for (int i = 0; i < channellist.size(); i++) {
						Map channelMap=channellist.get(i);
						String filePath=ParamsDatasUtil.getDownLoadPath(rybDownLoadPath, settleDate,"LT04");
						...
						String key=(String) channelMap.get("PRI_KEY");
						String fileName = prodMchId + "LT04_" + settleDate.replace("-", "");
						filePath = filePath + File.separator + fileName;
						log.info("获取申购文件存储到本地路径为： " + filePath);
						...
						builder.append("<CertId>DLDReq</CertId><FileType>LT04</FileType>");
	...
	public SettleSteps getSettleStep() {
		return SettleSteps.Deposit_Get_RYB;
	}
