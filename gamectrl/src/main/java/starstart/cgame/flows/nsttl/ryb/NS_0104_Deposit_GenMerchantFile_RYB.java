...
@Slf4j
@Data
public class NS_0104_Deposit_GenMerchantFile_RYB extends NSettle_BaseActor<CalcMutex> {
	...
	public void processMessage(CalcMutex pack) {
		...
		try {
			...
			String mntftpFilePath = ParamsDatasUtil.getDownLoadPath(filePath, settleDate, "101");
			...
			if (null != mchList && mchList.size() > 0) {
				for (int i = 0; i < mchList.size(); i++) {
					...
					String sql=CommonSql.getMSCS101FileSql(mchntId,settleDate,Constants.OFAG);
					List<HashMap> landMscsList = SerializerUtil.deserializeArray(LandMscs101Dao.doBySQL(sql), HashMap.class);
					String mntDeppsitFile = mntftpFilePath + mchntId + Constants.OFAG + settleDate.replace("-", "") + "101.txt";
					//数据写入指定文件内
			...
			throw new SettleException("生成给商户申购对账文件失败:", e);
		} finally {
			try {
				if (null != out) {
					out.close();
	...
	public SettleSteps getSettleStep() {
		return SettleSteps.Deposit_GenMerchantFile;
	}
