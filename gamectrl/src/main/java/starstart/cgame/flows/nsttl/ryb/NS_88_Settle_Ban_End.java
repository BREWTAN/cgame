...
@Slf4j
@Data
public class NS_88_Settle_Ban_End extends NSettle_BaseActor<CalcMutex>{
	...
	public void processMessage(CalcMutex pack) {
		try {
			...
			task.setTaskId(pack.getDatas().get(TaskDataDefs.PROD_MchID)+"|"+nextDay+"|STTL_00_BAN_OFAG");
			task.setCrtTime(new Date());
			task.setStatus(SettleStatus.Start.getCode());
			task.setTaskType("STTL_00_BAN_OFAG");
	...
	public SettleSteps getSettleStep() {
		return SettleSteps.End_Ban_OFAG;
	}
