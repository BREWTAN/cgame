...
/**
 * 
 * @ClassName: NS_0103_Deposit_Err_RYB
 * @Description: 差错文件处理
...
@Slf4j
@Data
public class NS_0103_Deposit_Err_RYB extends NSettle_BaseActor<CalcMutex> {
	...
	public void processMessage(CalcMutex pack) {
		...
		try {
			...
			String diffFilePath=ParamsDatasUtil.getDownLoadPath(dayOfEndFilePath, settleDate, "LT04");
	...
	public void updateAndInsertErr1(List<String> errStatusList)
		...
		try {
			...
			for (int i = 0; i < errStatusList.size(); i++) {
				...
				if (objs.length >= 4) {
					...
					if (null == land1List || land1List.size() == 0) {
						...
						String content = URLEncoder.encode("RYB申购差错对账通知prodOrderNo="+prodOrderNo, "utf8");
						...
						String info = "RYB申购差错对账通知prodOrderNo="+prodOrderNo;
						mailList.add(info);
			...
			if (mailList.size() > 0) {
				...
				String subject = StringUtil.getCurrDate()+"RYB申购差错对账通知";
				String text ="";
		...
		try {
			...
			for (int i = 0; i < errStatusList.size(); i++) {
				...
				if (null != mchntMap.get(prodOrderNo)) {
					...
					//更新 T_FRONT_USER_PROD_ACCT 余额值=原值+申购金额
...
//					settErr.setId(StringUtil.getUUID());
	...
	public SettleSteps getSettleStep() {
		return SettleSteps.Deposit_Err_RYB;
	}
