...
//			httpclient.getParams().setParameter(STALE_CONNECTION_CHECK, true);
	...
	public String post(String xml, String address, int timeoutMillis) throws ClientProtocolException, IOException {
		lock.readLock().lock();
		try {
//			httpclient.getParams().setParameter(SO_TIMEOUT, timeoutMillis);
...
//			httppost.getParams().setParameter(SO_TIMEOUT, timeoutMillis);
	...
	public String post(String xml, String address) throws ClientProtocolException, IOException {
		...
		try {
			...
			StringEntity entity = new StringEntity(xml, "UTF-8");
			HttpPost httppost = new HttpPost(address);
			httppost.addHeader("Content-Length", xml.length() + "");
	...
	public String postJson(String json, String address) throws ClientProtocolException, IOException {
		lock.readLock().lock();
		try {
			log.debug("httppost:" + address + ",data=" + json);
